...
void MyDialog::ShowLStatus(int port)
{
	...
	if (ret >= 0)
	{
		if (ret&S_CTS)
		{
			ctsState.SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
		...
		if (ret&S_DSR)
		{
			dsrState.SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
		...
		if (ret&S_RI)
		{
			RIState.SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
		...
		if (ret&S_CD)
		{
			dcdState.SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
