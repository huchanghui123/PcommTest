...
void ALLCOM::UpdateUI(void)
{
	...
	MoveWindow((x - cwidth) / 2, cr.top, cwidth, 300);
	...
	for (int i=0;i<COMCOUNT;i++)
	{
		...
		pEdit->Create(WS_CHILD | WS_VISIBLE | WS_BORDER | ES_MULTILINE | WS_VSCROLL,
			CRect(i * COMWIDTH + 10, 30, i * COMWIDTH + COMWIDTH, 200), this, i + 100);
		...
		ctsBtn->Create(_T("CTS"), WS_CHILD | WS_VISIBLE | WS_BORDER,
			CRect(i * COMWIDTH + 10, 210, i * COMWIDTH + 45, 230), this, i + 300);
		...
		dsrBtn->Create(_T("DTR"), WS_CHILD | WS_VISIBLE | WS_BORDER,
			CRect(i * COMWIDTH + 50, 210, i * COMWIDTH + 85, 230), this, i + 400);
		...
		dcdBtn->Create(_T("DCD"), WS_CHILD | WS_VISIBLE | WS_BORDER,
			CRect(i * COMWIDTH + 90, 210, i * COMWIDTH + 125, 230), this, i + 500);
...
void ALLCOM::ShowLStatus(int i)
{
	...
	if (ret >= 0)
	{
		if (ret&S_CTS)
		{
			ctsVector[i]->SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
		...
		if (ret&S_DSR)
		{
			dsrVector[i]->SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
		...
		if (ret&S_CD)
		{
			dcdVector[i]->SetFaceColor(RGB(255, 0, 0), TRUE);
		}
		else
		{
